...
public class ShopController {
    ...
    public ResponseEntity<List<ProductDTO>> getProducts (@RequestParam (name = "categoryId",required = false)Optional<String>categoryId,
                                                         ...
                                                         Pageable pageable){
        return ResponseUtil.page(productService.getProductsByFilters(categoryId,brand,minPrice,maxPrice,filter,pageable));
    }

    @GetMapping("/brand")
    public ResponseEntity<List<String>> getProductBrands (){
        return new ResponseEntity<>(productService.getProductBrands(),HttpStatus.OK);
    }
