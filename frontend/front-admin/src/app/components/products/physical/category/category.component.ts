...
export class CategoryComponent implements OnInit {
  ...
  categoryImageError = false;
  onFileChange(event: NgxDropzoneChangeEvent) {
    if (event.addedFiles.length === 0) {
      this.categoryImageError = true;
    } else {
      this.categoryImageError = false;
      // Proceed with handling the file upload
      const file = event.addedFiles[0];
      // You can perform further processing here, such as uploading the file
    }
  }
  ...
  saveNewCategory() {
    ...
    if (this.category?.id) {
      this.productService
        .editCateogry(this.category.id, categoryData)
        .subscribe((data) => {
          this.fetchData();
